...
LOGFILE_NAME="${LOGFILE_DIR}/Archiving_${OU}_DATA_FILE_$execution_date.log"
touch $LOGFILE_NAME
...
Archive_Dir="$WRAPPER_LOGS/DATA_FILES_CL3"
echo "$Archive_Dir"
...
##Common function for archiving activity
fun () {
    echo "Checking files under : $1"
    #arr_Filenames=($1/*.*)
    #FileCount=${#arr_Filenames[@]}
	FileCount=`find $1/ -type f -newermt "$StartDate" ! -newermt "$EndDate" | wc -l`
	echo "no of file to be archived with in the date range $StartDate to $EndDate is $FileCount"
    if [ $FileCount != 0 ]
	then
